...
public class VideoShader2D extends ShaderBase {
    private static final String TAG = "VideoShader2D";
    ...
    public VideoShader2D() {
        final String[] shaders = EvisUtil.get2DShader(EvisUtil.SHAER_TYPE_VIDEO);
        mProgram = ShaderHelper.buildProgramBinary(E3DApplication.getInstance().getApplicationContext(),
                    ShaderHelper.GLSL_VIDEO_2D, shaders[0], shaders[1]);
        if (mProgram == 0) {
